...
/* Head-to-head results use a diverging scale: the hue gives the direction of
   the record and the fill grows with |wins - losses| / games. The printed
   W-L remains the source of truth for readers who cannot distinguish color. */
.result{font-weight:650}
.result.pos{--result-color:var(--win)}
.result.neg{--result-color:var(--lose)}
.result.even{background:var(--chip);color:var(--ink-2)}
.result.pos.tone1,.result.neg.tone1{
  background:color-mix(in srgb,var(--result-color) 12%,transparent)}
.result.pos.tone2,.result.neg.tone2{
  background:color-mix(in srgb,var(--result-color) 21%,transparent)}
.result.pos.tone3,.result.neg.tone3{
  background:color-mix(in srgb,var(--result-color) 30%,transparent)}
.result.pos.tone4,.result.neg.tone4{
  background:color-mix(in srgb,var(--result-color) 39%,transparent)}
.result.pos.tone5,.result.neg.tone5{
  background:color-mix(in srgb,var(--result-color) 48%,transparent)}
.matrixwrap{overflow:auto;border:1px solid var(--line);border-radius:10px;
  background:var(--surface);max-height:72vh}
.matrixwrap table{border:0;border-radius:0;min-width:max-content}
.matrixwrap th,.matrixwrap td{white-space:nowrap}
.matrixwrap th.mxclub,.matrixwrap td.mxclub{position:sticky;left:0;z-index:2;
  min-width:210px;max-width:280px;overflow:hidden;text-overflow:ellipsis;
  background:var(--surface)}
.matrixwrap th.mxclub{z-index:4;background:var(--chip)}
.matrixwrap th.mxopp,.matrixwrap td.mxresult{width:52px;min-width:52px;
  text-align:center;padding-left:5px;padding-right:5px}
.matrixwrap th.mxopp{position:sticky;top:0;z-index:3}
.matrixwrap th.mxclub{top:0}
.matrixwrap td.mxself{color:var(--ink-3);background:var(--chip)}
.recordkey{display:inline-flex;align-items:center;gap:5px;margin-left:5px}
.recordkey i{display:inline-block;width:13px;height:13px;border-radius:3px}
.recordkey .good{background:color-mix(in srgb,var(--win) 42%,transparent)}
.recordkey .bad{background:color-mix(in srgb,var(--lose) 42%,transparent)}
...
<section id="rankings" class="on">
  ...
  <div id="clubRankings">
    <div class="bar">
      ...
      <select id="clubView" aria-label="Club rankings view">
        <option value="ranking">Ranking table</option>
        <option value="matrix">Results matrix</option>
      </select>
      <select id="basis">
...
function h2hCell(team, opponent, year, timeFrame = 'all') {
  ...
  const tone = record ? ` result ${recordTone(record)}` : '';
  const hidden = opponent ? '' : ' hidden';
  return `<td class="n h2hCell${tone}"${hidden} title="${title}">${body}</td>`;
}
function recordTone(record) {
  if (!record) return '';
  const balance = (record[0] - record[1]) / record[2];
...
function resultCell(team, opponent, year, timeFrame, rowName, opponentName) {
  ...
  if (!record)
    return `<td class="mxresult" title="No recorded games between ${esc(rowName)} and ` +
      `${esc(opponentName)}">—</td>`;
  const body = `${record[0]}–${record[1]}`;
  return `<td class="mxresult result ${recordTone(record)}" title="${esc(rowName)} ` +
...
function drawH2HMatrix(rows, rankOf, historical, year, timeFrame) {
  ...
  if (!HREADY) {
    matrix.innerHTML = `<p class="note">${HFAILED
      ...
      : 'Loading game results…'}</p>`;
    return 0;
  }
  // One club may rank in multiple divisions. A result belongs to the club
  // identity, so keep its highest-ranked appearance rather than drawing
  // duplicate rows and a false diagonal between them.
  const seen = new Set(), shown = [];
  for (const row of rows) {
    const key = h2hTeamKey(row, historical);
    if (seen.has(key)) continue;
    seen.add(key);
    shown.push({
      row, key, rank: rankOf.get(row),
      name: historical ? row.name : row.row[1],
      div: historical ? row.snap.div : row.row[5],
    });
    if (shown.length === 25) break;
  }
  const head = shown.map(x =>
    `<th class="mxopp n" scope="col" title="#${x.rank} ${esc(x.name)}">#${x.rank}</th>`
  ).join('');
  ...
  matrix.innerHTML = `<div class="matrixwrap"><table class="h2hmatrix">` +
    `<thead><tr><th class="mxclub" scope="col">Club</th>${head}</tr></thead>` +
...
function drawClubs() {
  ...
  if (matrixView) {
    ...
    $('#ccount').textContent = HREADY
      ...
      : (HFAILED ? 'Game results unavailable' : 'Loading game results…');
    $('#cnote').innerHTML =
      `Head-to-head records for ${year === 'all' ? 'all available seasons' : year}; ` +
      `the top 25 unique clubs in the selected ranking are shown. Search narrows ` +
      `the matrix. <span class="recordkey"><i class="good"></i>Winning record ` +
      `<i class="bad"></i>Losing record</span>. Stronger color means a more ` +
      `decisive record; tied and missing records remain neutral.`;
    return;
  }
